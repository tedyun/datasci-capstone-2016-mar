...
## predic the next text using the last "ngram" words in the text
## (hence docFreq should be (ngram + 1) frequency dictionary)
predict <- function (text, docFreq, ngram) {
  text  <- tolower(text)
  tokens <- removeFeatures(tokenize(text, removeNumbers = TRUE, removePunct = TRUE, removeSeparators = TRUE, removeTwitter = FALSE), stopwords(kind = "english", verbose = FALSE))
